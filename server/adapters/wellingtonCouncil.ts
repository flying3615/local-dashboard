...
const RSS_URL = "https://data-wcc.opendata.arcgis.com/api/feed/rss/2.0";
const SOURCE_URL = "https://data-wcc.opendata.arcgis.com/";
...
export interface WellingtonCouncilAdapterOptions {
  fetchImpl?: FetchImpl;
}
...
export function createWellingtonCouncilAdapter(
  options: WellingtonCouncilAdapterOptions = {},
): SourceAdapter {
  ...
  return {
    ...
    source: {
      name: "Wellington City Council Open Data",
    ...
    async fetch() {
      ...
      if (!response.ok) {
        throw new Error(
          `Wellington Council RSS request failed: ${response.status} ${response.statusText}`,
        );
      }
...
function mapRssItem(item: unknown) {
  ...
  return {
    title,
    summary: getString(item, "description") ?? "Wellington City Council open data update.",
    sourceUrl,
...
function buildTags(title: string): string[] {
  const tags = new Set(["wellington", "council", "open_data"]);
